...
modules live in `@rambleraptor/homestead-app` (the Next.js shell
package).
...
├── module.config.ts    # Module metadata (imports HomeModule from
│                       # @rambleraptor/homestead-app/registry/types)
...
The package gets bundled into the Next.js app via `createNextConfig`'s
`transpilePackages` list.

The list of modules served by an instance lives in the consumer's
`homestead.config.ts` (in-tree: `frontend/homestead.config.ts`; for a
self-hosted instance: the `homestead.config.ts` generated by
`create-homestead-app`). The registry imports it via the
`@homestead-config` TypeScript path alias. Routes are declared inline on
each `ModuleRoute` (the `component` field) and served by the catch-all
in `packages/homestead-app/pages/app/CatchAll.tsx`; do not create
per-route `page.tsx` files. See
...
`frontend/` is a **thin consumer** of `@rambleraptor/homestead-app`. The
only operator-facing files live here; every page/route is a one-line
re-export.

- `homestead.config.ts` — the ONE file you edit to pick modules
- `app/` — one-line re-exports from
  `@rambleraptor/homestead-app/pages/...` and
  `@rambleraptor/homestead-app/api/...` (Next.js App Router requires
  routes in the consumer's filesystem, but they hold no logic)
...
- `tsconfig.json` — extends `@rambleraptor/homestead-app/tsconfig.base.json`
  and sets the `@homestead-config` path alias to `./homestead.config.ts`

### Next.js shell package (`packages/homestead-app/`)

The `@rambleraptor/homestead-app` workspace package owns the Next.js
plumbing: layouts/providers, the module registry, the catch-all route
dispatcher, API route handlers, the server-side aepbase helper, the
schema-sync instrumentation hook, and a `createNextConfig` factory. See
`packages/homestead-app/README.md` for the directory layout.
...
package. Modules import the contract from
`@rambleraptor/homestead-app/registry/types` and shared building blocks
from `@rambleraptor/homestead-core/...`.

### Scaffolder (`packages/create-homestead-app/`)

A zero-dep Node CLI published to npm. `npx create-homestead-app <dir>`
copies a template (matching the in-tree `frontend/` consumer) into a new
directory, renames `_gitignore`/`_env.example` to their dotted forms,
stamps in the project name, and runs `npm install`. Keep the template
in sync with `frontend/` when you change the consumer shape.
...
At Next.js server boot, the instrumentation hook in
`packages/homestead-app/instrumentation/index.ts` (re-exported as the
consumer's `instrumentation.ts`) aggregates every declared definition
through `getAllResourceDefs()` plus `BUILTIN_RESOURCE_DEFS`,
topologically sorts by `parents`, and applies the result via aepbase's
`/aep-resource-definitions` endpoint. The runner
(`@rambleraptor/homestead-core/resources/sync.ts`) is idempotent: it
creates missing definitions, patches drifted ones, and no-ops when
...
server startup, the instrumentation hook in
`packages/homestead-app/instrumentation/index.ts` aggregates every
declared flag (via `getAllModuleFlagDefs` in
`packages/homestead-app/registry/registry.ts`), builds a JSON-schema
...
4. **Use the aepbase wrapper** (`@rambleraptor/homestead-core/api/aepbase`)
   ...
   `@rambleraptor/homestead-app/api/aepbase-server` for server routes.
