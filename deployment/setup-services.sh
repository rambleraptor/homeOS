...
# Copy and configure aepbase service
sed -e "s|/path/to/homeOS|$PROJECT_DIR|g" \
    -e "s|User=homeosuser|User=$CURRENT_USER|g" \
...
echo "  Start services:   sudo systemctl start homeos-pocketbase homeos-aepbase homeos-frontend"
echo "  Stop services:    sudo systemctl stop homeos-pocketbase homeos-aepbase homeos-frontend"
echo "  Restart services: sudo systemctl restart homeos-pocketbase homeos-aepbase homeos-frontend"
echo "  View status:      sudo systemctl status homeos-pocketbase homeos-aepbase homeos-frontend"
echo "  View logs:        sudo journalctl -u homeos-pocketbase -f"
echo "                    sudo journalctl -u homeos-frontend -f"
echo ""
echo "Next steps:"
echo "  1. Start services: sudo systemctl start homeos-pocketbase homeos-aepbase homeos-frontend"
