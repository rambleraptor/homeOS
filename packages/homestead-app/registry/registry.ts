 ...
 * Module Registry
 *
 * The list of modules served by this instance lives in the consumer
 * app's `homestead.config.ts`. Operators edit that file to choose which
 * modules to include; this file imports it via the `@homestead-config`
 * path alias and exposes navigation/flag/widget helpers over it.
 *
...
/**
 * Modules the registry always installs, regardless of what the
 * consumer's `homestead.config.ts` lists. These cover account management
 * and flag management — surfaces the rest of the app depends on.
 */
...
/**
 * Singleton instance of the module registry, built from the consumer
 * app's `homestead.config.ts` (resolved via the `@homestead-config`
 * alias) plus the always-installed core modules (superuser, settings).
 */
