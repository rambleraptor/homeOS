 ...
 *
 * This module is the authoritative decision core. Phase 1 lands the pure
 * resolver — `resolve()` (single-record / create decisions) and
 * `computeVisibility()` (the LIST predicate) — plus the kill-switch. Neither is
 * wired into the request path yet, and with `PERMISSIONS_ENFORCED` off the
 * engine behaves exactly as before. Later phases add the DB-backed
 * `PermissionStore` (which loads grants/roles/groups and feeds these functions)
 * and the enforcement wiring.
 *
 * Everything here is a pure function of its inputs, mirroring `access.ts`'s
 * `decide()`: easy to exhaustively unit-test, impossible to couple to request
 * plumbing by accident.
 */
 ...
 * merge dark: the resolver above can land and be exercised in tests without
 * changing observable behavior. Flip to `'true'` to enforce.
 */
