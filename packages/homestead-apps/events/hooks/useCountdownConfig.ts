/**
 * Read + write the events app's countdown configuration.
 *
...
export function useCountdownConfig(): UseCountdownConfigResult {
  const eventIdFlag = useUserSetting<string>(APP_ID, 'countdown_event_id');
  const months = useUserSetting<boolean>(APP_ID, UNIT_SETTING_KEY.months);
  const weeks = useUserSetting<boolean>(APP_ID, UNIT_SETTING_KEY.weeks);
  const days = useUserSetting<boolean>(APP_ID, UNIT_SETTING_KEY.days);
  const hours = useUserSetting<boolean>(APP_ID, UNIT_SETTING_KEY.hours);
  const minutes = useUserSetting<boolean>(APP_ID, UNIT_SETTING_KEY.minutes);
  const seconds = useUserSetting<boolean>(APP_ID, UNIT_SETTING_KEY.seconds);
