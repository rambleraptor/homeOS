/**
 * Omnibox intent parser (server side).
 *
 ...
 *
 * Runtime-agnostic: takes a Web `Request`, returns a Web `Response`, so
 * the Bun sidecar (and, transitionally, the Next route) can both use it.
 */

import { GoogleGenerativeAI } from '@google/generative-ai';
import { aepList, authenticate } from './aepbase';
import { buildManifest } from '../shared/omnibox/manifest';
import { parseFallback } from '../shared/omnibox/parseFallback';
import { fieldName } from '../settings/flags';
import type {
  OmniboxIntent,
...
async function omniboxAllowedForAll(token: string): Promise<boolean> {
  try {
    const records = await aepList<Record<string, unknown>>('module-flags', token);
...
/**
 * Parse an omnibox query from an authenticated request. Mirrors the old
 * `POST /api/omnibox/parse` Next route, runtime-agnostic.
 */
export async function parseOmniboxRequest(request: Request): Promise<Response> {
  try {
