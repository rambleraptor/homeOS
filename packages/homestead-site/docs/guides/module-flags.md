...
Module flags let your module add a typed, household-wide setting and read
or write it at runtime with a single hook. Use them for feature toggles,
per-module behavior switches, and other small bits of household-wide state
that don't deserve their own resource.

A flag is a typed value (`string`, `number`, `boolean`, or `enum`) you
declare in your module's `module.config.ts`. There's one stored value per
flag for the whole household, and you read or write it with the
`useModuleFlag` hook. Operators can also edit any flag from the superuser
...
- [Gating Your Module on Visibility](#gating-your-module-on-visibility)
- [Gotchas](#gotchas)
...
Flag declarations live on your module's `HomeModule` config under `flags`.
Each entry follows `ModuleFlagDef`:
...
export const exampleModule: HomeModule = {
  // ...
  flags: {
    theme: {
      type: 'enum',
...
The single public hook is `useModuleFlag` from
...
## Gating Your Module on Visibility
...
module's `defaultEnabled` config value (or `'all'` if unset):
...
| `tagged`     | Visible only to users whose account `tags` intersect the module's `enabled_tags` flag (any-of). |
...
To gate your UI on visibility, use `useIsModuleEnabled`:

```ts
import { useIsModuleEnabled } from '@rambleraptor/homestead-core/settings';
...
`useModuleEnabledPredicate`) handle the role/tag check against the
current user.
...
2. **Household-wide, not per-user.** All flags share one stored value for
   ...
   toggles", not user preferences. For per-user state, use a module's
...
4. **Don't redeclare `enabled` / `enabled_tags`.** They're auto-injected
   on every module and will override any declaration you add. Set
   `defaultEnabled` on the `HomeModule` config instead if you want a
   non-`'all'` default.
