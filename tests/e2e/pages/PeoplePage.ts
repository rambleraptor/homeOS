...
export class PeoplePage {
  ...
  async setNotificationPreference(timing: 'day_of' | 'day_before' | 'week_before', enabled: boolean) {
    ...
    const label = this.page.getByText(labels[timing], { exact: true });
    const checkbox = label.locator('..').locator('..').locator('input[type="checkbox"]');
  ...
  async setNotificationPreferences(preferences: Array<'day_of' | 'day_before' | 'week_before'>) {
    // First uncheck all
    for (const timing of ['day_of', 'day_before', 'week_before'] as const) {
      await this.setNotificationPreference(timing, false);
    }
    // Then check the desired ones
    for (const timing of preferences) {
      await this.setNotificationPreference(timing, true);
    }
  }

  async expectNotificationPreference(timing: 'day_of' | 'day_before' | 'week_before', enabled: boolean) {
    const labels: Record<string, string> = {
      'day_of': 'On the day',
      'day_before': 'The day before',
      'week_before': 'A week before',
  ...
  async createPersonWithNotifications(data: {
    name: string;
    birthday?: string;
    anniversary?: string;
    notificationPreferences: Array<'day_of' | 'day_before' | 'week_before'>;
  }) {
    await this.clickAddPerson();
    await this.page.locator('#name').fill(data.name);
    if (data.birthday) {
    ...
    await this.setNotificationPreferences(data.notificationPreferences);
    await this.submitPersonForm();
    await this.page.waitForLoadState('networkidle');
  }

  async editPersonNotifications(personName: string, newPreferences: Array<'day_of' | 'day_before' | 'week_before'>) {
    await this.expectPersonInList(personName);
    ...
    await this.page.locator('#name').waitFor({ state: 'visible' });
    await this.setNotificationPreferences(newPreferences);
    await this.submitPersonForm();
  }
}
