 ...
 * household has flipped the `settings.omnibox_access` setting to
 * `'all'`. The value is backed by the shared module-settings hook so
 * it updates reactively across the app when a superuser changes it.
 */

'use client';

import { useAuth } from '@/core/auth/useAuth';
import { useModuleSetting } from '@/modules/settings/hooks/useModuleSetting';
import type { OmniboxAccess } from '@/modules/settings/module.config';

export function useCanUseOmnibox(): boolean {
  const { user } = useAuth();
  const { value } = useModuleSetting<OmniboxAccess>(
    'settings',
    'omnibox_access',
  );
