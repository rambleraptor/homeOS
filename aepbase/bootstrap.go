...
// Credentials are the superuser credentials for this aepbase instance.
type Credentials struct {
	Email    string
	Password string
}

// bootstrapSuperuser handles the first-boot superuser creation that upstream
// aepbase normally does inside EnableUsers (and prints to stdout where the
// launcher can't recover it). We create the user ourselves so the credentials
// can be persisted to data/credentials.json for the launcher to read and pass
// to the sidecar for the schema-sync step.
//
// Flow:
//
//   - If credentials.json exists in dataDir, reuse it (whether or not the user
//     actually exists in the DB).
//   - Otherwise count users. If zero, generate a password, insert the
//     superuser with the same shape EnableUsers would have used, and persist
//     credentials.json. EnableUsers then sees count > 0 and skips its bootstrap.
//   - If users exist but credentials.json is missing, return an error — we have
